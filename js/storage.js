...
class InvoiceStorage {
    constructor() {
        ...
        this.KEY_PRODUCTS = 'invoice-generator-products';
        this.KEY_COMPANY_TEMPLATES = 'invoice-generator-company-templates';
    }
    ...
    saveInvoice(invoiceData) {
        try {
            ...
            if (error.name === 'QuotaExceededError') {
                throw new Error('Storage full — cannot save invoice. Please free up space or export old invoices.');
            }
            return false;
        }
    }
     ...
     * Use for display purposes only (form initialization, preview)
     */
     ...
     * Use only when actually saving a new invoice
     */
    ...
    getProducts() {
        try {
            const json = localStorage.getItem(this.KEY_PRODUCTS);
    ...
    saveProduct(productData) {
        try {
            ...
            if (!productData.name || !productData.name.trim()) return false;
            ...
            if (existingIndex > -1) {
                ...
                products.push({
                    ...productData,
                    id: productData.id || InvoiceStorage.generateId(),
                    createdAt: new Date().toISOString(),
                    updatedAt: new Date().toISOString()
                });
            }
    ...
    deleteProduct(productId) {
        try {
            ...
            localStorage.setItem(this.KEY_PRODUCTS, JSON.stringify(filtered));
            return deleted;
        } catch (error) {
            return false;
        }
    ...
    /**
     * Get all company templates from storage
     */
    getCompanyTemplates() {
        try {
            const templatesJson = localStorage.getItem(this.KEY_COMPANY_TEMPLATES);
    ...
    /**
     * Save a company template to storage
     */
    saveCompanyTemplate(templateData) {
        try {
            if (!templateData || typeof templateData !== 'object') return false;

            const templates = this.getCompanyTemplates();
            const data = JSON.parse(JSON.stringify(templateData));

            if (!data.id) {
                data.id = InvoiceStorage.generateId();
            }

            const existingIndex = templates.findIndex(t => t.id === data.id);
            if (existingIndex > -1) {
                templates[existingIndex] = { ...data, updatedAt: new Date().toISOString() };
            } else {
                templates.push({
                    ...data,
                    createdAt: new Date().toISOString(),
            ...
            localStorage.setItem(this.KEY_COMPANY_TEMPLATES, JSON.stringify(templates));
            return data.id;
        } catch (error) {
            return false;
        }
    ...
    /**
     * Delete a company template from storage
     */
    deleteCompanyTemplate(templateId) {
        try {
            ...
            localStorage.setItem(this.KEY_COMPANY_TEMPLATES, JSON.stringify(filtered));
            return deleted;
        } catch (error) {
            return false;
        }
