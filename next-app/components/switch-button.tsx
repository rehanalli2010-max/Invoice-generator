...
export default function SwitchButton({
  ...
  const variants = {
    ...
    square: [
      ...
      "backdrop-blur-sm",
      "relative",
      "after:absolute after:inset-0 after:rounded-xl after:bg-gradient-to-t after:from-white/10 after:to-transparent after:opacity-0 hover:after:opacity-100 after:transition-opacity",
      "before:absolute before:inset-[1px] before:rounded-[9px] before:bg-gradient-to-b before:from-white/20 before:to-transparent before:opacity-0 hover:before:opacity-100 before:transition-opacity dark:before:from-white/5",
    ],
  };
  ...
  return (
    <Button
      className={cn(
        "group relative flex items-center justify-center p-0",
        "transition-all duration-300 ease-out",
        "text-zinc-600 dark:text-zinc-300",
        "hover:text-zinc-800 dark:hover:text-zinc-100",
        variants[variant].filter((v) => !v.startsWith("w-") && !v.startsWith("h-")),
        isSquare ? squareSizes[size || "default"] : sizes[size || "default"],
        className
      )}
        ...
        <Sun
          className={cn(
            ...
            isSquare
              ? "h-5 w-5"
              : size === "sm" && "h-3.5 w-3.5",
            !isSquare && size === "default" && "h-4 w-4",
            !isSquare && size === "lg" && "h-5 w-5",
